...
int sys_fork()
{
  ...
  init_stats(childproc->statistics);
  childproc->quantum = 15;
  
  union task_union *childunion = (union task_union*) childproc;
  childunion->stack[KERNEL_STACK_SIZE-19] = 0; //fake
...
{  
	...
	update_process_state_rr(exitproc, &freequeue); //list_add_tail(&(current()->list), &freequeue);??
	sched_next_rr();
}
